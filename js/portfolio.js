...
function createPortfolioCard(image, artistSlug) {
    ...
    card.setAttribute('data-style', image.style);
    card.setAttribute('data-image-id', image.id);
    card.setAttribute('tabindex', '0');
    card.setAttribute('role', 'button');
    
    // Add tag data attributes for filtering
    ...
    const healingStatus = image.isHealed ? 'healed' : 'fresh';
    const coverupText = image.beforeImage ? ', cover-up work' : '';
    const ariaLabel = `View ${image.title}, ${image.style} tattoo on ${image.placement}, ${healingStatus}${coverupText}. Session time: ${image.sessionTime}. Press Enter or Space to open detailed view.`;
    card.setAttribute('aria-label', ariaLabel);
    card.setAttribute('aria-describedby', `portfolio-card-desc-${image.id}`);
    
    // Add ARIA pressed state for interaction feedback
    card.setAttribute('aria-pressed', 'false');
    
    // Create image container with lazy loading
    const imageContainer = document.createElement('div');
    imageContainer.className = 'portfolio-image loading';
    
    const img = document.createElement('img');
    img.setAttribute('data-src', `../images/portfolio/${artistSlug}/${image.filename}`);
    
    // Comprehensive alt text for accessibility
    const artistName = ArtistManager.getArtistBySlug(artistSlug)?.name || artistSlug;
    const healingStatusAlt = image.isHealed ? 'healed' : 'fresh';
    const coverupTextAlt = image.beforeImage ? ' (cover-up work)' : '';
    const altText = `${image.title}: ${image.style} tattoo on ${image.placement} by ${artistName}. ${healingStatusAlt} tattoo${coverupTextAlt}. ${image.description || ''}`.trim();
    ...
    // Create overlay with image information
    const overlay = document.createElement('div');
    overlay.className = 'portfolio-overlay';
    
    // Build overlay content with conditional elements
    let overlayContent = `
        <h3>${image.title}</h3>
        <div class="overlay-tags">
            <div class="style-tag">${image.style}</div>
            ${image.isHealed ? '<div class="healed-tag">Healed</div>' : '<div class="fresh-tag">Fresh</div>'}
            ${image.beforeImage ? '<div class="coverup-tag">Cover-up</div>' : ''}
        </div>
        <div class="overlay-details">
            <div class="placement"><span class="detail-icon">📍</span>${image.placement}</div>
            <div class="session-time"><span class="detail-icon">⏱️</span>${image.sessionTime}</div>
        </div>
    `;
    
    // Add description if available
    if (image.description && image.description.length > 0) {
        const shortDescription = image.description.length > 60 
            ? image.description.substring(0, 60) + '...' 
            : image.description;
        overlayContent += `<div class="overlay-description">${shortDescription}</div>`;
    }
    ...
    hiddenDesc.id = `portfolio-card-desc-${image.id}`;
    hiddenDesc.className = 'sr-only';
    hiddenDesc.textContent = `${image.description || 'No additional description available.'}`;
    
    card.appendChild(imageContainer);
    card.appendChild(overlay);
    card.appendChild(hiddenDesc);
    
    // Add click handler with accessibility feedback
    card.addEventListener('click', () => {
        card.setAttribute('aria-pressed', 'true');
        handlePortfolioCardClick(image, artistSlug);
        setTimeout(() => card.setAttribute('aria-pressed', 'false'), 200);
    });
...
function createLightboxHTML() {
    const lightboxHTML = `
        <div class="lightbox-overlay" role="dialog" aria-modal="true" aria-labelledby="lightbox-title" aria-describedby="lightbox-description">
            <div class="lightbox-container">
                <button class="lightbox-close" aria-label="Close lightbox and return to portfolio" title="Close (ESC)">
                    <span aria-hidden="true">&times;</span>
                </button>
                
                <button class="lightbox-nav lightbox-prev" aria-label="View previous portfolio image" title="Previous image (Left Arrow)" aria-describedby="lightbox-nav-help">
                    <span aria-hidden="true">‹</span>
                </button>
                
                <button class="lightbox-nav lightbox-next" aria-label="View next portfolio image" title="Next image (Right Arrow)" aria-describedby="lightbox-nav-help">
                    <span aria-hidden="true">›</span>
                </button>
                
                <div id="lightbox-nav-help" class="sr-only">Use arrow keys to navigate between images, or press Escape to close</div>
                
                <div class="lightbox-content">
                    <div class="lightbox-image-container" role="img" aria-labelledby="lightbox-title">
                        <img class="lightbox-image" src="" alt="" role="img" />
                        <div class="lightbox-loading" aria-label="Loading image" role="status" aria-live="polite">
                            <div class="loading-spinner"></div>
                            <span class="sr-only">Loading tattoo image...</span>
                        </div>
                    </div>
                    
                    <div class="lightbox-info">
                        <h2 id="lightbox-title" class="lightbox-title"></h2>
                        <div id="lightbox-description" class="sr-only"></div>
                        <div class="lightbox-metadata" role="list" aria-label="Tattoo details">
                            <div class="metadata-item" role="listitem">
                                <span class="metadata-label">Style:</span>
                                <span class="metadata-value lightbox-style"></span>
                            </div>
                            <div class="metadata-item" role="listitem">
                                <span class="metadata-label">Placement:</span>
                                <span class="metadata-value lightbox-placement"></span>
                            </div>
                            <div class="metadata-item" role="listitem">
                                <span class="metadata-label">Session Time:</span>
                                <span class="metadata-value lightbox-session-time"></span>
                            </div>
                            <div class="metadata-item lightbox-healing-status" role="listitem">
                                <span class="metadata-label">Status:</span>
                                <span class="metadata-value lightbox-healed"></span>
                            </div>
                        </div>
                        <div class="lightbox-before-after" style="display: none;">
                            <div class="before-after-toggle">
                                <button class="toggle-btn active" data-view="after">After</button>
                                <button class="toggle-btn" data-view="before">Before</button>
                            </div>
                            <div class="before-after-images">
                                <div class="before-image-container" style="display: none;">
                                    <img class="lightbox-before-image" src="" alt="" />
                                    <div class="image-label">Before</div>
                                </div>
                            </div>
                        </div>
                        <p class="lightbox-description" id="lightbox-full-description"></p>
                        <div class="lightbox-counter" role="status" aria-live="polite" aria-label="Image position in portfolio">
                            <span class="sr-only">Image </span><span class="current-image">1</span> <span class="sr-only">of </span><span class="total-images">1</span>
                        </div>
                    </div>
                </div>
            </div>
...
function updateLightboxContent() {
    ...
    // Update image with accessibility enhancements
    ...
    announceToScreenReader('Loading tattoo image...');
    
    // Set image source with comprehensive alt text
    const imageSrc = `../images/portfolio/${currentArtistSlug}/${currentLightboxImage.filename}`;
    lightboxImage.src = imageSrc;
    
    const artistName = ArtistManager.getArtistBySlug(currentArtistSlug)?.name || currentArtistSlug;
    const healingStatusLightbox = currentLightboxImage.isHealed ? 'healed' : 'fresh';
    const coverupTextLightbox = currentLightboxImage.beforeImage ? ' (cover-up work)' : '';
    const altText = `${currentLightboxImage.title}: ${currentLightboxImage.style} tattoo on ${currentLightboxImage.placement} by ${artistName}. ${healingStatusLightbox} tattoo${coverupTextLightbox}. ${currentLightboxImage.description || ''}`.trim();
    
    lightboxImage.alt = altText;
    
    // Handle image load with accessibility announcement
    lightboxImage.onload = () => {
        loadingSpinner.style.display = 'none';
        lightboxImage.style.opacity = '1';
        announceToScreenReader(`Image loaded: ${currentLightboxImage.title}`);
    };
    ...
    lightboxImage.onerror = () => {
        ...
        // Show error message in image container
    ...
    // Update metadata with accessibility enhancements
    lightbox.querySelector('.lightbox-title').textContent = currentLightboxImage.title;
    lightbox.querySelector('.lightbox-style').textContent = currentLightboxImage.style;
